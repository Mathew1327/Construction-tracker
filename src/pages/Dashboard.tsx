...
export function Dashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const { data: materials } = await supabase.from("materials").select("qty_required");
            ...
            {clientProjects.map((project) => (
              <div key={project.id} className="bg-white p-4 rounded-lg shadow border">
                ...
                <div className="mt-3 space-y-2">
                  {clientPhases.filter((p) => p.project_id === project.id).map((phase) => (
                    <div key={phase.id} className="p-3 border rounded bg-gray-50">
                      <p className="font-medium">{phase.name}</p>
                      <p className="text-sm text-gray-600">Status: {phase.status}</p>
